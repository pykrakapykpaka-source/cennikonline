...
export default function Lead({
  ...
  return (
    <>
      ...
      {true && (
        ...
        >
          ...
          <div className="flex w-full justify-between items-center">
            ...
            {(lead?.leadType === "cenniki" || lead?.intention) && (
              <div className="mr-2 hidden sm:block">
                <span className="text-xs font-bold px-2 py-1 rounded bg-white/10 border border-white/10">
                  {lead?.intention || "Cenniki"}
                </span>
              </div>
            )}
            <button
              ...
              {(lead?.leadType === "cenniki" || lead?.intention) && (
                <>
                  <tr className="bg-gray-700">
                    <td>Intencja:</td>
                    <td>{lead?.intention || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-600">
                    <td>Branża:</td>
                    <td>{lead?.industry || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-700">
                    <td>Ilość usług:</td>
                    <td>{lead?.servicesCount || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-600">
                    <td>Model wyceny:</td>
                    <td>{lead?.pricingModel || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-700">
                    <td>Format:</td>
                    <td>{lead?.outputFormat || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-600">
                    <td>Materiały:</td>
                    <td>{lead?.contentStatus || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-700">
                    <td>Termin:</td>
                    <td>{lead?.deadline || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-600">
                    <td>Budżet:</td>
                    <td>{lead?.budget || "Nie podano"}</td>
                  </tr>
                  <tr className="bg-gray-700">
                    <td>Kontakt:</td>
                    <td>
      ...
      {/* NIKODEM LEADS (deprecated - access is now handled by NEXT_PUBLIC_ADMIN_EMAILS) */}
      {false && (
          <div
            key={lead.id}
            className={`rounded relative bg-zinc-800 p-3 h-max border-[3px] overflow-hidden ${
      ...
      {/* FORCE VISIBILITY ON MAIN ADMIN (deprecated - access is now handled by NEXT_PUBLIC_ADMIN_EMAILS) */}
      {false && (
        <div
          key={lead.id}
          className={`rounded relative bg-zinc-800 p-3 h-max border-[3px] overflow-hidden ${
