...
let auth = null;
/** @type {import('firebase/firestore/lite').Firestore | null} */
let db = null;
...
function initFirebase() {
  if (app) return { app, provider, auth, db, storage, analytics };
  ...
  try {
    ...
    return { app, provider, auth, db, storage, analytics };
  } catch (error) {
    // During build, if env vars are not set, return null values
    if (process.env.NODE_ENV === 'production' && !process.env.NEXT_PUBLIC_FIREBASE_API_KEY) {
      return { app: null, provider: null, auth: null, db: null, storage: null, analytics: null };
    }
    throw error;
  }
}

// Try to initialize, but don't fail if config is missing during build
...
try {
  ...
  if (process.env.NODE_ENV !== 'production' || !process.env.NEXT_PUBLIC_FIREBASE_API_KEY) {
    // Build time - Firebase not configured, that's ok
  } else {
    throw error;
  }
}
...
export async function pushQuestion(data) {
  const productDocRef = doc(collection(db, "questions"), data.id);
  const docSnap = await getDoc(productDocRef);
  if (docSnap.exists()) {
    return productDocRef;
  } else {
